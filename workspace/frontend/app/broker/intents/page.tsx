...
import { fetchBrokerIntents } from "@/features/properties/api";
import { extractError } from "@/lib/api";
import { formatKRW } from "@/lib/format";
import type { PurchaseIntent } from "@/lib/types";
import { useAuthStore } from "@/stores/auth";

export default function BrokerIntents() {
  ...
  return (
    <div className="mx-auto max-w-page px-6 py-10 space-y-6">
      <h1 className="text-3xl font-bold">수신 의향서</h1>
      <p className="text-sm text-text-muted">내 매물에 들어온 매수 의향서 목록입니다.</p>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs text-text-muted">
                    매수자 ID: {it.buyer_id.slice(0, 8)}… · 제출 {new Date(it.created_at).toLocaleString("ko-KR")}
                  </p>
                  {it.message && (
                ...
                <div className="text-right">
                  ...
                  {it.desired_close_date && (
                    <p className="mt-1 text-[11px] text-text-muted">희망 클로징: {it.desired_close_date}</p>
                  )}
                </div>
              </div>
            </li>
